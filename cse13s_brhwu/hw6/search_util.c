...
int score_letter(char letter, char **vocabulary, size_t num_words) {
  ...
  for (size_t i = 0; i < num_words; i++) {
      ...
      if (strchr(vocabulary[i], letter) != NULL) { //if letter is in word
          num ++;
      }
  }
  return num;
...
int score_word(char *word, int *letter_scores) {
  ...
  int score = 0;
  for (int j = 0; j < 26; j++) {
      char current_letter = 'a' + j;
      if (strchr(word, current_letter) != NULL) { //if letter is in word
          score = score + letter_scores[j];
      }
  }
  return score;
...
size_t filter_vocabulary_yellow(char letter, int position, char **vocabulary,
  ...
  for (size_t i = 0; i < num_words; i++) {
      ...
      //if word does not have letter or has letter in position
...
size_t filter_vocabulary_green(char letter, int position, char **vocabulary,
  ...
  for (size_t i = 0; i < num_words; i++) {
      ...
      if (vocabulary[i][position] != letter) { //if word does not have letter in position
          free(vocabulary[i]);
          vocabulary[i] = NULL;
          num++;
      }
